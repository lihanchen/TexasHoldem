 ...
 * 
 * @author kevin (modified by wangyix)
 */
public class MyMouseOverArea extends AbstractComponent {
	/** The default state */
	 ...
	 */
	public MyMouseOverArea(GUIContext container, Image image, int x, int y, ComponentListener listener) {
	 ...
	 */
	public MyMouseOverArea(GUIContext container, Image image, Shape shape) {
		super(container);
	...
	private void updateImage() {
		if (!over) {
		...
		else {	// we're now in button area
			
			if (mouseDown) {			// over and mouse down

				if (state==NORMAL) {	// mouse passes into button area while down (does not press button)
			...
			} else {					// over and mouse up
				
				currentImage = mouseOverImage;
				currentColor = mouseOverColor;
				
				if (state==MOUSE_DOWN) {			// mouse down-to-up transition in button area
					if (mouseDownSound != null) {
						mouseDownSound.play();
					}
					notifyListeners();
				}
				else if (state==NORMAL) {			// mouse passes into button area while up
